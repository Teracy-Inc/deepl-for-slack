...
app.message('', async ({ body, client }) => {
    ...
    if (replies.messages && replies.messages.length > 0) {
        const message = replies.messages[0]
        if (message.text) {
            const formattedText = message.text.replace(/<(.*?)>/g, '')
            const proposedLangs = lngDetector
                .detect(formattedText, 3)
                .map((val: any[][]) => val[0])
            const lang = proposedLangs.length ? 'ja' : 'en'
            const translatedText = await deepL.translate(formattedText, lang)
